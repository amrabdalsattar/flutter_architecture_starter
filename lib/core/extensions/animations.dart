...
extension AnimationsExtension on Widget {
  ...
  Widget fadeInScale({
    ...
    return animate(delay: delay.ms)
        ...
        .scale(duration: scaleDuration.ms, curve: curve);
  }

  /// Fade in only (simple and clean)
  Widget fadeInOnly({int delay = 0, int duration = 400}) {
  ...
  Widget formFieldAnimation({int index = 0}) {
    return fadeInSlide(delay: index * 100, duration: 500, slideOffset: 0.2);
  }

  /// List item animation pattern
  /// Used for list items with stagger effect
  Widget listItemAnimation({int index = 0, int baseDelay = 0}) {
    return fadeInSlide(
  ...
  Widget slideUpFromBottom({
    ...
    return animate(
      delay: delay.ms,
    ).slideY(begin: 1.0, end: 0, duration: duration.ms, curve: curve);
  }
  ...
  Widget bounce({int delay = 0, int duration = 600}) {
  ...
  Widget pulse({int delay = 0, double scale = 1.1}) {
    return animate(delay: delay.ms)
        .scale(
  ...
  Widget pop({int delay = 0, int duration = 200}) {
    return animate(delay: delay.ms).scale(
  ...
  Widget rotateIn({int delay = 0, int duration = 600, double rotation = 0.25}) {
    return animate(delay: delay.ms)
        .fadeIn(duration: duration.ms)
        .rotate(
          begin: rotation,
  ...
  Widget spin({int duration = 1000}) {
    return animate(
      onComplete: (controller) {
        controller.repeat();
      },
    ).rotate(duration: duration.ms);
  }
  ...
  Widget successAnimation({int delay = 0}) {
    return animate(delay: delay.ms)
  ...
  Widget errorAnimation({int delay = 0}) {
    return animate(delay: delay.ms)
  ...
  Widget attentionSeeker({int delay = 0}) {
    return animate(delay: delay.ms)
        .fadeIn(duration: 400.ms)
        .then()
        .scale(
  ...
  Widget flipIn({
    ...
    return animate(delay: delay.ms)
        .fadeIn(duration: duration.ms)
        .flip(duration: duration.ms, curve: Curves.easeOut);
  }
  ...
  Widget blurIn({int delay = 0, int duration = 500, double blur = 5.0}) {
